      ...
      <div className="portfolio--section--container">
        {data?.portfolio?.map((item, index) => (
          <div key={index} className="portfolio--section--card">
            <div className="portfolio--section--img"  onClick={() => window.location.href = item.diploy}>
              <img src={item.src} alt="Placeholder" />
            </div>
            <div className="portfolio--section--card--content">
              <div>
                <h3 className="portfolio--section--title">{item.title}</h3>
                <p className="text-md overflow-ellipsis">{item.description}</p>
              </div>
              <p className="text-sm portfolio--link" onClick={() => window.location.href = item.Github}> 
                {item.link}
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
